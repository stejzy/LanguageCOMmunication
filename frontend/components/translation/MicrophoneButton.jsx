...
export default function MicrophoneButton({ isRecording, setIsRecording}) {
   ...
   const setupAudioWorklet = (source) => {
      ...
      audioContext.current.audioWorklet.addModule(moduleURL)
          .then(() => {
              workletNode.current = new AudioWorkletNode(audioContext.current, "pcm-worklet");
              ...
              source.connect(workletNode.current).connect(audioContext.current.destination);
          })
    ...
    return(
        <Pressable style={styles.micButton} onPress={toggleRecording} disabled={disableMic}>
            <FontAwesome
